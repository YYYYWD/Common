...
/*-----动态调度------*/ 
 void schedule()
 {
     ...
     struct tcb*curr_select=g_task_running;
     while(select!=NULL)//先遍历一次链表
     {
         /*--PPT上有说明要计算所有线程的priority的更新--*/
     ...
     while(select!=NULL)
     {
         if((select->tid!=0)&&(select->state==TASK_STATE_READY))//如果不是系统空闲进程，且该进程的状态是就绪态的话
         {
             if(select->priority>curr_select->priority)
             {
                 curr_select=select;//选择就绪队列里动态优先级最高的
             }
             if(curr_select->tid==0) //只有一开始curr_select指向了task0才可能出现这样的情况
                 curr_select=select;
         }
         select=select->next;
     }
      if(curr_select == g_task_running) {
          if(curr_select->state == TASK_STATE_READY)
              return;
          curr_select = task0;
      }
      g_resched = 0;
      switch_to(curr_select);
 }
/**
