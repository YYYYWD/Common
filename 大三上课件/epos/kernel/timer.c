...
void isr_timer(uint32_t irq, struct context *ctx)
{
    ...
    if(g_task_running != NULL) {
        ...
        if(g_task_running->tid == 0) {
            ...
            if((g_timer_ticks % HZ==0))     //过一秒计算一次
            {
                int nready=0;//nready表示处于就绪状态的线程个数
                struct tcb *curr_select=g_task_head;
                int nice;
                fixedpt ratio;
                while(curr_select!=NULL)    //遍历整个链表
                {
                    if(curr_select->state==TASK_STATE_READY)nready++;
                    nice=curr_select->nice;
                    //每秒钟为所有进程更新一次
                    ratio=fixedpt_mul(FIXEDPT_TWO, g_load_avg);
                    ratio=fixedpt_div(ratio,fixedpt_add(ratio,FIXEDPT_ONE));
                    curr_select->estcpu=fixedpt_add(fixedpt_mul(ratio,curr_select->estcpu),fixedpt_fromint(nice));
