...
void syscall(struct context *ctx)
{
    ...
    switch(ctx->eax) {
    case SYSCALL_DDAline :
    {
        int x0 = *(int *)(ctx->esp+4);
        int y0 = *(int *)(ctx->esp+8);
        int x1 = *(int *)(ctx->esp+12);
        int y1 = *(int *)(ctx->esp+16);
        COLORREF cr=*(int *)(ctx->esp+20);
        ctx->eax=sys_DDAline(x0,y0,x1,y1,cr);
    }
	case SYSCALL_time:
	{
    ...
    break;
	//break machdep.c:589	setting a break point at line 589
...
{
	time_t time_now = g_timer_ticks/HZ + g_startup_time;
	return time_now;
}
int sys_DDAline(int x0,int y0,int x1,int y1,COLORREF cr)
{
...
{
    if(tid==0)
        return g_task_running->nice+NZERO;//获取当前线程的nice
    uint32_t flags;  struct tcb *tsk;
    save_flags_cli(flags);
    tsk = get_task(tid);
    restore_flags(flags);
    if(tsk!=NULL)//如果线程不为null
    {
        if(tsk->nice>=-NZERO&&tsk->nice<NZERO)
        {
...
int sys_setpriority(int tid, int prio)
{
    ...
    {       
        save_flags_cli(flags);
        g_task_running->nice=prio-20;
        restore_flags(flags);
        return 0;
    }
    /*---防止用户设置的时候越界---*/
    if(prio<0)prio=0;
    if(prio>40)prio=2*NZERO-1;  
    save_flags_cli(flags);
    tsk = get_task(tid);
    restore_flags(flags);
    if(tsk==NULL)
